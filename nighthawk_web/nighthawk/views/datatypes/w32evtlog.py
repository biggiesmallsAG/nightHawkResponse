...
class W32EvtLog(View):
	def __init__(self):
		pass

	def get(self, request, case=None, hostname=None):
		assert isinstance(request, HttpRequest)
		return render(request, 'w32evtlogs.html',
	...
	def post(self, request, case=None, hostname=None):
		if request.method == 'POST' and request.is_ajax():
			...
			q = QueryES()
			data = q.GetAuditData(case, hostname, "w32eventlogs", start=start, length=length, str_query=search_val, sort=sort, order=order)
