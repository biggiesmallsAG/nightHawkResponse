...
export class NhUploadComponent implements OnInit {
	...
	private pageTitle : string = "Upload & Delete";
	private selectedCases;
	private selectedEndpoints;
	private uploadResponse;
	private deleteCaseForm: FormGroup;
	private deleteEndpointForm: FormGroup;
	private _deleteEndpointFromCase: boolean = false;
	...
	private setCase(obj, model: string) {
		switch (model) {
			case "case":
			if (obj != "") {
	...
	private deleteCase(model: DeleteCase, isValid: boolean, event: Event) {
	...
	private deleteEndpoint(model: DeleteEndpoint, isValid: boolean, event: Event) {
		event.preventDefault();
		this._nHLoader.show();
		this._nhCoreSvc.POSTJSON(model.path, {
	...
	private deleteEndpointFromCase() {
		this._nHLoader.show();
	...
	private save(model: UploadHandler, isValid: boolean, event: Event) {
		event.preventDefault();
		this.submitted = true;
