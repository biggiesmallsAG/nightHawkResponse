...
func Login(w http.ResponseWriter, r *http.Request) {
	...
	if !boolUserExits {
		api.HttpResponseReturn(w, r, "failed", fmt.Sprintf("%s - Username does not exist", loginData["username"]), nil)
		return
	}
	...
	if passwordMatched {
		...
		api.HttpResponseReturn(w, r, "failed", fmt.Sprintf("%s - Password did not match", loginData["username"]), nil)
	}
}
