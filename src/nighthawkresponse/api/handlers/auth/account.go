...
func (config *Account) Validation() error {
	//// Username validation
	// Username must be 3 or more characters
	if len(config.Username) < 3 {
		return errors.New("Username must of at least 3 characters")
	}
	...
	usernameReg, err := regexp.Compile("[^a-zA-Z0-9]+")
	if err != nil {
		return err
	}
	if usernameReg.MatchString(config.Username) {
	...
	if config.Role != "admin" && config.Role != "user" {
...
func (config *Account) LoadParams(data []byte) {
	...
	if tconfig.Username != "" {
		config.Username = strings.ToLower(tconfig.Username)
	}
	if tconfig.Password != "" {
	...
	if tconfig.Email != "" {
		config.Email = strings.ToLower(tconfig.Email)
	}
	if tconfig.Role != "" {
		config.Role = strings.ToLower(tconfig.Role)
	}
}
...
func DeleteUser(w http.ResponseWriter, r *http.Request) {
	...
	username := strings.ToLower(postData["username"])

	validUser, acc := UserExists(username)
	if !validUser {
		api.HttpResponseReturn(w, r, "failed", err.Error(), nil)
		return
	}
...
func SetPassword(w http.ResponseWriter, r *http.Request) {
	...
	validUser, acc := UserExists(username)
	if !validUser {
		api.HttpResponseReturn(w, r, "failed", "User not found", username)
		return
	}
	...
	out, err := client.Update().Index(NHINDEX).Type(NHACCOUNT).Id(acc.DocId).Doc(map[string]interface{}{"password_hash": newPasswordHash}).Do(context.Background())
	if err != nil || out.Result != "updated" {
		api.HttpResponseReturn(w, r, "failed", err.Error(), username)
		return
	}
	api.HttpResponseReturn(w, r, "success", "Password set completed", username)
}
