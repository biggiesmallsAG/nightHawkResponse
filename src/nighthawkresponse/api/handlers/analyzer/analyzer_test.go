...
func TestShowAnalyzerItemByByType(t *testing.T) {
	item := AnalyzeItem{
		AuditType: "w32prefetch",
	}
	jitem, _ := json.Marshal(item)
	data := bytes.NewReader(jitem)

	req, err := http.NewRequest("POST", "/api/v1/analyze/show/blacklist", data)
	...
	r := mux.NewRouter()
	r.HandleFunc("/api/v1/analyze/show/{analyzer_type}", ShowAnalyzerItemByType)
