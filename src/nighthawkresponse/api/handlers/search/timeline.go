...
func GetTimelineSearch(w http.ResponseWriter, r *http.Request) {
	...
	if len(tc.ComputerList) > 0 {
		...
		query = elastic.NewBoolQuery().
			Must(elastic.NewWildcardQuery("CaseInfo.CaseName.keyword", tc.CaseName),
	...
	api.HttpResponseReturn(w, r, "success", "Timeline search data", tl_data)
	//Bug(2018-01-28) - Error formating Task Path containing %windir
	// Change from api.HttpSuccessMessage to api.HttpResponseReturn
	/*
