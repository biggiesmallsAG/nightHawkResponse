/* nighthawk.audit.parser.files.go
 * author: 0xredskull
 *
	...
	for {
		...
		case xml.StartElement:
			...
			if inElement == "FileItem" {
				...
				if item.Modified == "" {item.Modified = nhutil.FixEmptyTimestamp()}
				if item.Accessed == "" {item.Accessed = nhutil.FixEmptyTimestamp()}
				if item.Changed == "" {item.Changed = nhutil.FixEmptyTimestamp()}
				if item.FilenameCreated == "" {item.FilenameCreated = nhutil.FixEmptyTimestamp()}
				if item.FilenameModified == "" {item.FilenameModified = nhutil.FixEmptyTimestamp()}
