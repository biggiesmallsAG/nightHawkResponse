/* nighthawk.audit.parser.filedlhistory.go
 * author: 0xredskull
 *
	...
	for {
		...
		case xml.StartElement:
			...
			if inElement == "FileDownloadHistoryItem" {
				...
				// Fix empty timestamp
				if item.LastModifiedDate == "" {item.LastModifiedDate = nhutil.FixEmptyTimestamp()}
				if item.LastCheckedDate == "" {item.LastCheckedDate = nhutil.FixEmptyTimestamp()}
