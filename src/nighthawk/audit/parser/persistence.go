	...
	for {
		...
		case xml.StartElement:
			...
			if inElement == "PersistenceItem" {
				...
				if item.RegModified == "" { item.RegModified = nhutil.FixEmptyTimestamp() }
				if item.FileModified == "" { item.FileModified = nhutil.FixEmptyTimestamp()}
				if item.FileCreated == "" { item.FileCreated = nhutil.FixEmptyTimestamp()}
				if item.FileAccessed == "" {item.FileAccessed = nhutil.FixEmptyTimestamp()}
				if item.FileChanged == "" {item.FileChanged = nhutil.FixEmptyTimestamp()}
				if item.File.JobCreated == "" { item.File.JobCreated = nhutil.FixEmptyTimestamp() }
				if item.File.Created == "" { item.File.Created = nhutil.FixEmptyTimestamp() }
				if item.File.Modified == "" { item.File.Modified = nhutil.FixEmptyTimestamp() }
				if item.File.Accessed == "" { item.File.Accessed = nhutil.FixEmptyTimestamp() }
				if item.File.Changed  == "" { item.File.Changed = nhutil.FixEmptyTimestamp() }
				if item.Registry.JobCreated == "" { item.Registry.JobCreated = nhutil.FixEmptyTimestamp() }
				if item.Registry.Modified == "" { item.Registry.Modified = nhutil.FixEmptyTimestamp() }
				if item.File.PeInfo.PETimeStamp == "" { item.File.PeInfo.PETimeStamp = nhutil.FixEmptyTimestamp() }
				...
				if fStatus {
					...
					item.File.NhComment.Analyst = "nighthawk"
