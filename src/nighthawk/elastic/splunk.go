...
func UploadToSplunk(computername string, audit string, data []nhs.RlRecord) {
	...
	upload_url := fmt.Sprintf("https://%s:%d/services/receivers/simple?source=nighthawk&sourcetyp=redline&output_mode=json",nhconfig.ElasticHost(), nhconfig.ElasticPort())
